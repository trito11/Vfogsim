...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [CellMatrix, UserMatrix_New]= Default_CellUpdate(CellMatrix, UserMatrix_Old, UserMatrix_New, CellID_loc, UserDemand_Location, Is_Blocked, IsCompleted, RemTime_loc, Cell_Change_loc)
    ...
    UserMatrix_New(user, Cell_Change_loc) = UserMatrix_Old(user, Cell_Change_loc);
       if(UserMatrix_Old(user,CellID_loc) ~= UserMatrix_New(user,CellID_loc)) 
           UserMatrix_New(user, Cell_Change_loc) = UserMatrix_New(user, Cell_Change_loc) + 1; % Migrating the task
       end
       %% If the user is blocked
        if(UserMatrix_Old(user,Is_Blocked)==1)
            UserMatrix_New(user,RemTime_loc) = UserMatrix_Old(user,RemTime_loc);
        else
            if(UserMatrix_Old(user,CellID_loc)~=0 && UserMatrix_Old(user,UserDemand_Location)>0)  % Is served
