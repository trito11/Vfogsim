...
def Default_UtilityFunction_Edge(UserList, price_list, OBJSIZE, USERCOUNT, Resource_Demand_loc, Remaining_Time_loc, Demand_ServiceType_loc):
    ...
    for k in range(USERCOUNT):
        if UserList[k][Remaining_Time_loc ] > 0:
            const[k][k] = -price_list[UserList[k][Demand_ServiceType_loc ]] * UserList[k][Resource_Demand_loc ] * 1 / math.log(UserList[k][Remaining_Time_loc ])
        else:
