...
def QoS_Cell_based(Cell_ID_loc, Ttr, Tmig, Texec, Cell_Change_loc, Demand_Time_loc, Queue_Delay_loc, UserMatrix):
    ...
    Cells = np.zeros(np.max(dummy))
    Cell_trace = np.zeros((dummy.shape[1], np.max(dummy)))
    Cell_trace_users = np.zeros((dummy.shape[1], np.max(dummy)))
    ...
    T = Cells * 100 / np.sum(Cell_trace_users != 0, axis=1)
    return T