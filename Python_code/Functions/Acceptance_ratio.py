...
def acceptance_ratio(Unserved, events):
    Tot_Users = Unserved.shape[1]
    # AcceptanceRatio = np.zeros((1, Tot_Users))
    Tot_requests = np.zeros((1, Tot_Users))
    
    for l in range(events.shape[0]):
        if events[l, 0] < np.max(events[:, 0]) and events[l, 0] > 1:
            Tot_requests[0, events[l, 1]] += 1
